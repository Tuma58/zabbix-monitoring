...
Целевой UX (React + TypeScript) сохраняет 5 шагов:

```text
Основное → Подключение → Проверка → Рекомендация → Применение
```

Текущий vanilla-прототип в [`dashboard/`](../dashboard/) использует 4 шага, где
«Рекомендация» и «Применение» объединены в финальный экран «Готово»:

```text
Основное → Подключение → Проверка → Готово
```

При миграции на React шаги снова разделяются; контракт probe/plan/apply в API
уже рассчитан на полный pipeline.
